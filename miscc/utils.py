...
def compute_discriminator_loss_cpu(netD, real_imgs, fake_imgs,
                               real_labels, fake_labels, real_catelabels,
                               conditions):
    ratio = 1.0
    cate_criterion =nn.MultiLabelSoftMarginLoss()
    ...
    # 使用多卡gpu这么写
    # real_features = nn.parallel.data_parallel(netD, (real_imgs)).to(device)
    # fake_features = nn.parallel.data_parallel(netD, (fake)).to(device)
    real_features = netD(real_imgs)
    fake_features = netD(fake)

    # real pairs
    inputs = (real_features, cond)
    # real_logits = nn.parallel.data_parallel(netD.get_cond_logits, inputs).to(device)
    real_logits = netD.get_cond_logits(real_features, cond)

    errD_real = criterion(real_logits, real_labels)
    # wrong pairs
    inputs = (real_features[:(batch_size-1)], cond[1:])
    # wrong_logits = nn.parallel.data_parallel(netD.get_cond_logits, inputs).to(device)
    wrong_logits = netD.get_cond_logits(real_features[:(batch_size-1)], cond[1:])

    errD_wrong = criterion(wrong_logits, fake_labels[1:])
    # fake pairs
    inputs = (fake_features, cond)
    # fake_logits = nn.parallel.data_parallel(netD.get_cond_logits, inputs).to(device)
    fake_logits = netD.get_cond_logits(fake_features, cond)
    errD_fake = criterion(fake_logits, fake_labels)

    if netD.get_uncond_logits is not None:
        # real_logits = nn.parallel.data_parallel(netD.get_uncond_logits, real_features).to(device)
        # fake_logits = nn.parallel.data_parallel(netD.get_uncond_logits, fake_features).to(device)
    ...
    acc = 0
    if netD.cate_classify is not None:
        # cate_logits = nn.parallel.data_parallel(netD.cate_classify, real_features).to(device)
